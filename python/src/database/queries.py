...
class AdverseEventQueries(BaseQueries):
    ...
    def create(
        ...
        # Récupérer le prochain numéro d'EI pour ce patient
        cursor.execute(
        ...
        cursor.execute("""
            ...
            (patient_id, ae_number, start_date, description, severity, seriousness, causality, is_serious)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """, (patient_id, ae_number, start_date, description, severity, seriousness, causality, int(is_serious)))
        self.conn.commit()
        return cursor.lastrowid
